...
if (!function_exists('upload_media_in_storage')) {
    ...
    /**
     * The function upload_media_in_storage uploads media in the user storagez
     * 
     * @return void
     */
            ...
            if ( ( $CI->form_validation->run() === false ) && empty($_FILES['file']['name'])  ) {
                ...
                if ( in_array( $_FILES['file']['type'], $check_format ) ) {
                    ...
                    if ( $CI->upload->do_upload('file') ) {
                        ...
                        update_user_meta( $CI->user_id, 'user_storage', $total_storage );
                        ...
                        if ( $type === 'video' ) {
                            ...
                            $last_id = $CI->media->save_media($CI->user_id, $CI->config->base_url() . 'assets/share/' . $data['upload_data']['file_name'], 'video', $cover, $_FILES['file']['size']);

                        } else {
                            
                            // Save uploaded file data
                            $last_id = $CI->media->save_media($CI->user_id, $CI->config->base_url() . 'assets/share/' . $data['upload_data']['file_name'], 'image', $cover, $_FILES['file']['size']);
...
if (!function_exists('save_media_in_storage')) {
    ...
    function save_media_in_storage() {
        ...
        if ( $CI->input->post() ) {
            ...
            $CI->form_validation->set_rules('size', 'Size', 'trim|required');
...
if (!function_exists('get_media')) {
    ...
    function get_media() {
        ...
        $total = $CI->media->get_user_medias($CI->user_id);
        
        $getmedias = $CI->media->get_user_medias($CI->user_id, $page * $limit, $limit);
...
if (!function_exists('delete_media')) {
    ...
    function delete_media( $media_id = NULL, $returns = NULL ) {
        ...
        // Get file info
        $file_info = $CI->media->single_media($CI->user_id, $media_id);
