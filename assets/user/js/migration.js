...
jQuery(document).ready(function ($) {
	...
	jQuery('.new_migrationsetting').submit(function () {
        ...
        var data = {
			migration_id: '',
            opencart_website_url: jQuery('.opencart_websiteurl').val(),
            opencart_database_name: jQuery('.opencart_database').val(),
            opencart_database_username: jQuery('.opencart_dbuser').val(),
            opencart_database_password: jQuery('.opencart_dbpassword').val(),
            opencart_database_host: jQuery('.opencart_dbhost').val(),
			opencart_database_prefix: jQuery('.opencart_dbprefix').val(),
			opencart_website_admin_username: jQuery('.opencart_admin').val(),
			opencart_website_admin_password: jQuery('.opencart_admin_password').val(),
            magento_website_url: jQuery('.magento_websiteurl').val(),
			magento_database_name: jQuery('.magento_database').val(),
			magento_database_username: jQuery('.magento_dbuser').val(),
			magento_database_password: jQuery('.magento_dbpassword').val(),
			magento_database_host: jQuery('.magento_dbhost').val(),
			magento_database_prefix: jQuery('.magento_dbprefix').val(),
			magento_website_admin_username: jQuery('.magento_admin').val(),
			magento_website_admin_password: jQuery('.magento_admin_password').val(),
            csrf_test_name: name,
                ...
                if (data) {
                  ...
                  //  show_pagination(data.total, '.fl');
                    ...
                    for (var u = 0; u < data.settings.length; u++) {
                        ...
                        allusers += '<div class="col-lg-12"><div><b>Magento Web URL</b>: '+ data.settings[u].magento_website_url + '</br> <b>Opencart Web URL</b>: ' + data.settings[u].opencart_website_url + '</div>' + edit + '</div>';
                    }
    ...
    function migration_edit() {
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                if (data.msg) {
                    jQuery('.opencart_websiteurl').val(data.msg.opencart_website_url);
                    jQuery('.opencart_database').val(data.msg.opencart_database_name);
                    jQuery('.opencart_dbuser').val(data.msg.opencart_database_username);
                    jQuery('.opencart_dbpassword').val(data.msg.opencart_database_password);
                    jQuery('.opencart_dbhost').val(data.msg.opencart_database_host);
                    jQuery('.opencart_dbprefix').val(data.msg.opencart_database_prefix);
                    jQuery('.opencart_admin').val(data.msg.opencart_website_admin_username);
					jQuery('.opencart_admin_password').val(data.msg.opencart_website_admin_password);
					jQuery('.magento_websiteurl').val(data.msg.magento_website_url);
					jQuery('.magento_database').val(data.msg.magento_database_name);
					jQuery('.magento_dbuser').val(data.msg.magento_database_username);
					jQuery('.magento_dbpassword').val(data.msg.magento_database_password);
					jQuery('.magento_dbhost').val(data.msg.magento_database_host);
					jQuery('.magento_dbprefix').val(data.msg.magento_database_prefix);
					jQuery('.magento_admin').val(data.msg.magento_website_admin_username);
					jQuery('.magento_admin_password').val(data.msg.magento_website_admin_password);
                }
            },
            error: function (data, jqXHR, textStatus) {
                //console.log('Request failed: ' + textStatus);
