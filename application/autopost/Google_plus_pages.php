 ...
 * PHP Version 5.6
 *
 * Connect and Publish to Google Plus Pages
 *
 * @category Social
 * @package  Midrub
...
/**
 * Google Plus Pages class - allows users to connect to their Google Plus Pages and publish posts by using Buffer api.
 *
 * @category Social
 * @package  Midrub
 * @author   Scrisoft <asksyn@gmail.com>
 ...
 * @link     https://www.midrub.com/
 */
class Google_plus_pages implements Autopost {
    ...
    public function __construct() {
        ...
        $this->client_id = get_option('google_plus_pages_client_id');
        
        // Get Buffer client_secret
        $this->client_secret = get_option('google_plus_pages_client_secret');
        
        // Get Buffer redirect
        $this->redirect_uri = $this->CI->config->base_url() . 'admin/callback/google_plus_pages';
        
        // Get user role
        $this->role = $this->CI->user->check_role_by_username($this->CI->session->userdata['username']);
        
        // Verify if the social network is enabled
        if ( get_option( 'google_plus_pages' ) ) {
        ...
        if ( !$this->role ) {
            ...
            if ( $this->CI->input->post() ) {
                ...
                if ( $this->CI->form_validation->run() == false ) {
                    ...
                    echo $this->CI->ecl('Social_login_connect')->view($this->CI->lang->line('social_connector'), '<p class="alert alert-error">' . $this->CI->lang->line('mu385') . '</p>', false); 
                    
                } else {
                    
                    $this->CI->db->select( '*' );
                    $this->CI->db->from( 'users_networks' );
                    $this->CI->db->where(['type' => 'google_plus_pages', 'email' => $email]);
                    $query = $this->CI->db->get();
                    if ( $query->num_rows() > 0 ) {
                        
                        // Display the error message
                        echo $this->CI->ecl('Social_login_connect')->view($this->CI->lang->line('social_connector'), '<p class="alert alert-error">' . $this->CI->lang->line('mu386') . '</p>', false); 
                        ...
                        if ( $this->CI->db->affected_rows() ) {
                            ...
                            // Display the error message
                            echo $this->CI->ecl('Social_login_connect')->view($this->CI->lang->line('social_connector'), '<p class="alert alert-error">' . $this->CI->lang->line('mu390') . '</p>', false); 
            ...
            echo $this->CI->ecl('Social_login')->content('Email', 'Password', 'Connect', $this->get_info(), 'google_plus_pages', $this->CI->lang->line('mu325'));
            ...
            if ( @$token['access_token'] ) {
                ...
                if ( $profiles && isset($_SESSION['net_id']) ) {
                    ...
                    foreach ( $profiles as $profile ) {
                        ...
                        if ( $profile->formatted_service == 'Google+ Page' && $user_id > 0 ) {
                            
                            // Verify if account was already saved 
                            if ( !$this->CI->networks->check_account_was_added( 'google_plus_pages', $profile->_id, $user_id ) ) {
                                
                                $network_id = $this->CI->networks->add_network( 'google_plus_pages', $profile->_id, $token['access_token'], $user_id, '', $profile->formatted_username, $profile->avatar );
    ...
    /**
     * The public method post publishes posts on Google Plus Pages
     *
    ...
    public function post($args, $user_id = null) {
        ...
        if ( $user_id ) {
            ...
            $con = $this->CI->networks->get_network_data(strtolower('google_plus_pages'), $user_id, $args ['account']);
        ...
        if ( $con ) {
            ...
            if ( $con [0]->token ) {
                ...
                try {
                    ...
                    if ( $args['img'] ) {
                        $params['media'] = array('photo' => $args['img'][0]['body']);
                    } 
                    
                    // Verify if url exists
                    if ( $args['url'] ) {
                        $post = str_replace($args['url'], ' ', $post);
                        $params['media'] = array('link' => short_url($args['url']));
                    }                    
    ...
    public function get_info() {
        ...
        if ( ($this->role > 0) && ($this->CI->db->table_exists('users_networks') == true ) ) {
            ...
            $this->CI->db->select( '*' );
            $this->CI->db->from( 'users_networks' );
            $this->CI->db->where( ['type' => 'google_plus_pages'] );
            ...
            if ( $query->num_rows() > 0 ) {
                ...
                foreach ( $results as $result ) {
                    ...
                    if ( !$result->status ) {
                        ...
                        $connect = '<a class="btn btn-success btn-xs"  href="' . base_url( 'admin/connect/google_plus_pages?net_id=' . $result->net_id ) . '" style="margin-top: -2px; color: #ffffff; margin-left: 0;"><i class="fa fa-plug" aria-hidden="true"></i></span></a>';
                    ...
                    $users .= '<tr>
                                    ...
                                    <td class="text-center"><a class="btn btn-danger btn-xs" href="' . base_url( 'admin/network/google_plus_pages?page=' . $page . '&delete=' . $result->net_id ) . '" style="margin-top: -2px;"><span class="glyphicon glyphicon-trash"></span></a></td>
                                </tr>';
                }
            }
            ...
            if ( $current > 1 ) {
                ...
                $bac = $current - 1;
                $pages = '<li><a href="' . base_url( 'admin/network/google_plus_pages?page=' . $bac ) . '">' . $this->CI->lang->line('mm129') . '</a></li>';
            ...
            for ( $p = $from; $p < $tot; $p++ ) {
                ...
                if ( $p == $current ) {
                    ...
                    $pages .= '<li><a href="' . base_url( 'admin/network/google_plus_pages?page=' . $p ) . '">' . $p . '</a></li>';
            ...
            if ( $p == 1 ) {
                ...
                $pages .= '<li class="active"><a href="' . base_url( 'admin/network/google_plus_pages?page=' . $p ) . '">' . $p . '</a></li>';
            ...
            if ( $next < $tot ) {
                ...
                $pages = $pages . '<li><a href="' . base_url( 'admin/network/google_plus_pages?page=' . $next ) . '">' . $this->CI->lang->line('mm128') . '</a></li>';
    ...
    /**
     * This function generates a preview for Google Plus Pages.
     *
