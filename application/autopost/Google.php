        ...
        if ( !$this->role ) {
            ...
            if ( $this->CI->input->post() ) {
                ...
                if ( $this->CI->form_validation->run() == false ) {
                    ...
                    $this->CI->db->where(['type' => 'google', 'email' => $email]);
                    $query = $this->CI->db->get();
                    if ( $query->num_rows() > 0 ) {
                        
                        // Display the error message
                        echo $this->CI->ecl('Social_login_connect')->view($this->CI->lang->line('social_connector'), '<p class="alert alert-error">' . $this->CI->lang->line('mu388') . '</p>', false);
