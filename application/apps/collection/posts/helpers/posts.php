            ...
            if ( $this->CI->form_validation->run() === false ) {
                ...
                if ( is_numeric($group_id) ) {
                    ...
                    if ( $metas ) {
                        ...
                        $data = array(
                            'success' => FALSE,
                            'message' => $this->CI->lang->line('mm192')
                        );
                ...
                $lastId = $this->CI->posts_model->save_post($this->CI->user_id, $post, $url, $img, $video, $date, $publish, $category, $post_title);
                                        ...
                                        foreach ($accounts as $ac_id) {
                                            ...
                                            if ( (int)$publish === 1 ) {
                                                ...
                                                if ( get_user_option('use_spintax_posts') ) {
                                                    ...
                                                    if ( in_array($network, $this->CI->socials) ) {
                                ...
                                foreach ($networks as $meta) {
                                    ...
                                    if ( file_exists(APPPATH . 'autopost/' . ucfirst($meta->network_name) . '.php') ) {
                                        ...
                                        if ( $this->CI->user->get_user_option($this->CI->user_id, 'use_spintax_posts') === 1 ) {
                                            ...
                                            if ( in_array($meta->network_name, $this->CI->socials) ) {
                                                ...
                                                $this->CI->socials[] = $meta->network_name;
                        ...
                        if ( $net ) {
                            ...
                            if ( (int)$publish === 1 ) {
                                ...
                                if ( $this->CI->options->check_enabled('enable-scheduled-notifications') ) {
                                    ...
                                    $args = ['[site_name]' => $this->CI->config->item('site_name'), '[login_address]' => '<a href="' . $this->CI->config->item('login_url') . '">' . $this->CI->config->item('login_url') . '</a>', '[site_url]' => '<a href="' . $this->CI->config->base_url() . '">' . $this->CI->config->base_url() . '</a>'];
