...
/*
 * Facebook_my_groups class loads the insigts
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.0
 */
class Facebook_my_groups implements MidrubAppsCollectionPostsInterfaces\Insights {
    ...
    public function get_account($network) {
        ...
        $posts = json_decode( get( $this->url . $network[0]->net_id . '/feed?access_token=' . $network[0]->token ) );
        ...
        if ( @$posts->data ) {
            ...
            foreach ( $posts->data as $post  ) {
                ...
                $all_posts[] = array(
                    ...
                    'created_time' => (@$post->created_time)?$post->created_time:$post->updated_time,
    ...
    public function get_comments($network) {
        ...
        $comments = json_decode( get( $this->url . $network[0]->post_id . '/comments?access_token=' . $network[0]->token ) );
        ...
        if ( @$comments->data ) {
            ...
            foreach ( $comments->data as $comment ) {
                ...
                $replies = json_decode( get( $this->url . $comment->id . '/comments?access_token=' . $network[0]->token ) );
    ...
    public function get_insights($network, $type) {
        ...
        switch ( $type ) {
            ...
            case 'account':
                ...
                // Get members
                $members = json_decode( get( $this->url . $network[0]->net_id . '/?fields=member_count&access_token=' . $network[0]->token ) );
                ...
                // Get requests
                $requests = json_decode( get( $this->url . $network[0]->net_id . '/?fields=member_request_count&access_token=' . $network[0]->token ) );

                $requests_count = 0;

                if ( @$requests->member_count ) {
    ...
    public function delete($network, $type, $parent = NULL) {
        ...
        switch ($type) {
            ...
            case 'comments':
                ...
                $post = json_decode(delete( $this->url . $parent, $network[0]->token));
            ...
            case 'post':
                ...
                $post = json_decode(delete( $this->url . $network[0]->post_id, $network[0]->token));
