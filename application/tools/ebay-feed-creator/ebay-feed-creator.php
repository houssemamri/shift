...
class Ebay_feed_creator implements Tools {
    ...
    public function page($args) {
        ...
        $CI = & get_instance();
        $content = '<p class="no-results-found">'.$CI->lang->line('mu60').'</p>'.$CI->lang->line('mu118');
        $url = '';
        $title = '';
        $show = '';
            ...
            if ((@$curl == @$url) && (@$title != '')) {
                ...
                if(($all_rss?count($all_rss):0) < $rss_limit) {
                    ...
                    $data = array(
                        ...
                        'rss_url' => site_url('user/tools/ebay-feed-creator').'?rss-url='.base64_encode($url).'&tool='.$this->check_info()->slug,
                                    ...
                                    <li>
                                        ...
                                        ' . form_open('user/tools/ebay-feed-creator') . '
                                            <div class="input-group search">
                                                <input type="text" name="feed-url" placeholder="'.$CI->lang->line('mu117').'" value="' . $url . '" class="form-control rss-url" required="true">
                            ...
                            <li>
                                ...
                                ' . form_open('user/tools/ebay-feed-creator') . '
                                    <div class="input-group search">
                                        ...
                                        <input type="hidden" name="curl" value="' . $url . '">
                                        <span class="input-group-btn search-m">
                                            <button class="btn save-rss" type="submit" type="button"' . $show . '><i class="fas fa-save"></i></button>
                                            <button class="btn parse" type="submit"><i class="icon-cloud-download"></i></button>
                                        </span>
                                    </div>
                                    ' . form_close() . '
