...
if (!function_exists('custom_header')) {
    ...
    function custom_header() {
        ...
        $data = '<link rel="stylesheet" type="text/css" href="' . base_url() . 'assets/admin/css/style.css?ver=' . MD_VER . '" media="all"/>';
        
        $data .= "\n";
        
        if ( $CI->router->fetch_method() === 'admin_apps' ) {
            $data .= '<link rel="stylesheet" type="text/css" href="' . base_url() . 'assets/admin/css/apps.css?ver=' . MD_VER . '" media="all"/> ';
            $data .= "\n";
        }
        
        if ( $CI->router->fetch_method() === 'admin_plans' ) {
            $data .= '<link rel="stylesheet" type="text/css" href="' . base_url() . 'assets/admin/css/plans.css?ver=' . MD_VER . '" media="all"/> ';
            $data .= "\n";
        }

        if ( $CI->router->fetch_method() === 'all_tickets' || $CI->router->fetch_method() === 'new_faq_article' || $CI->router->fetch_method() === 'faq_articles' ) {
            $data .= '<link rel="stylesheet" type="text/css" href="' . base_url() . 'assets/admin/css/tickets.css?ver=' . MD_VER . '" media="all"/> ';
            $data .= "\n";
        }
