...
class Faq extends CI_MODEL {
     ...
     */
    public function get_faq_articles($type, $start = NULL , $limit = NULL) {
        ...
        $this->db->join('users', 'users.user_id=users.user_id', 'left');
    ...
    /**
     * The public method get_faq_articles_by_search gets faq articles by key
     * 
     * @param string $key contains the search's key
     * 
     * @return object with articles or false
     */
    public function get_faq_articles_by_search($key) {
        
        $language = $this->config->item('language');
        
        $where = array(
            'faq_articles_meta.language' => $language,
            'faq_articles.status' => 1 
        );
    ...
    /**
     * The public method get_faq_articles_by_category gets faq articles by category
     * 
     * @param string $category_id contains the category's id
     * 
     * @return object with articles or false
     */
    public function get_faq_articles_by_category($category_id) {
        
        $language = $this->config->item('language');
        
        $where = array(
            'faq_articles_categories.category_id' => $category_id,
            'faq_articles.status' => 1
        );
        ...
        $this->db->join('users', 'users.user_id=users.user_id', 'left');
        $this->db->join('faq_articles_meta', 'faq_articles.article_id=faq_articles_meta.article_id', 'left');
        $this->db->join('faq_articles_categories', 'faq_articles.article_id=faq_articles_categories.article_id', 'left');
        $this->db->where($where);
        $this->db->group_by('faq_articles.article_id');
        $this->db->order_by('faq_articles.article_id', 'desc');
    ...
    /**
     * The public method get_category_meta gets category's meta
     * 
     * @param integer $category_id contains the category's id
     * @param string $language contains the category's language
     * 
     * @return object with category's meta or false
     */
    public function get_category_meta($category_id, $language) {
        
        $this->db->select('faq_categories.*');
        $this->db->from('faq_categories_meta');
        $this->db->join('faq_categories', 'faq_categories_meta.category_id=faq_categories.category_id', 'left');
        $this->db->where(array(
                'faq_categories_meta.category_id' => $category_id,
                'faq_categories_meta.language' => strtolower($language)
            )
        );
